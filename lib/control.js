...
const debug = dbg('leaflet-transit-editor:control')
...
export default class TransitEditorControl extends L.Control {
  ...
  onAdd (map) {
    ...
    save.addEventListener('click', (e) => {
      // prevent invalid layer save
      if (this.layer.isValid()) this.save(this.layer.getModification())
      else debug('Layer is not valid, refusing to save')
      this.close()
    })
    ...
    /** only show the checkbox if we can change which end we're extending from */
    if (this.layer.allowExtendFromEnd && this.layer.allowExtendFromStart) {
