...
export default class TransitEditorLayer extends L.LayerGroup {
  ...
  constructor ({ geometry, stops, controlPoints }) {
    ...
    this.handleSegmentDrag = this.handleSegmentDrag.bind(this)
  ...
  handleMarkerDrag (e) {
    ...
    // TODO: reflow stops around marker
  ...
  handleSegmentDrag (e) {
    ...
    // it's a control point but not a stop
    this.splice(coord, this.draggingSegment, true, false)
  ...
  getMarker (coord, controlPoint, stop) {
    ...
    let marker = L.marker(L.latLng(coord[1], coord[0]), { icon, draggable: true })
  ...
  reflow (point) {
    ...
    // if it's a stop and a control point, restart the reflowing here.
    if (anchored) this.reflowFrom(newPointIndex)
  }
  ...
  reflowFrom (pointIdx) {
    ...
    for (let segIdx = pointIdx + 1; ; segIdx++) {
      ...
      if (this.controlPoints[segIdx] || !this.stops[segIdx]) coords.push(this.segments[segIdx].geometry.coordinates[0])
    ...
    let outCoords = [coords[0]]
    let outStops = [stops[0]]
    let outControlPoints = [controlPoints[0]]
    ...
    while (segment < distances.length) {
      ...
      if (next < total + distances[segment]) {
        ...
        // SOMETHING ISN'T RIGHT HERE. outStops ends up all true, even when stops is not all true.
